...
public class Gray8AffineWarp1Pass extends PipelineStage {   
     ...
     * @throws Error if the warp is null or not a 2x3 matrix.
     */
    public Gray8AffineWarp1Pass(double[][] warp) throws Error {
        this.setWarp(warp);
    }
     ...
     * @throws Error if the warp is not 2x3 or the warp is not
    ...
    public void setWarp(double[][] warp) throws Error {
        if (warp.length != 2 || warp[0].length != 3 || warp[1].length != 3) {
            throw new Error(
                            Error.PACKAGE.ALGORITHM,
                            ErrorCodes.PARAMETER_WRONG_SIZE,
                            warp.toString(),
                            null,
                            null);
        }
